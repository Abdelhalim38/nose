...
for domain in ${cache_domains}; do
	"${awk_tool}" -v d="${domain}" '$0~d{print $0}' "./output/Face_ipv4.txt" >>"./Face_ipv4_cache.tmp"
	"${awk_tool}" -v d="${domain}" '$0~d{print $0}' "./output/Face_ipv6.txt" >>"./Face_ipv6_cache.tmp"
done
...
while IFS= read -r domain; do
	(
				...
				for ip in ${ips}; do
					if [ "${ip%%.*}" = "127" ] || [ "${ip%%.*}" = "0" ] || [ -z "${ip%%::*}" ]; then
						...
						if ipcalc-ng -cs "${ip}"; then
							domain_ok="true"
							if [ "${ip##*:}" = "${ip}" ]; then
								printf "%-20s%s\n" "${ip}" "# ${domain}" >>"./Face_ipv4.tmp"
							else
								printf "%-40s%s\n" "${ip}" "# ${domain}" >>"./Face_ipv6.tmp"
							fi
						fi
					fi
				done
		...
		if [ "${domain_ok}" = "false" ]; then
			printf "%s\n" "${domain}" >>./Face_domains_abandoned.tmp
		else
			printf "%s\n" "${domain}" >>./Face_domains.tmp
		fi
	) &
	hold1="$((cnt % 512))"
	hold2="$((cnt % 2048))"
...
#
sort -b -u -n -t. -k1,1 -k2,2 -k3,3 -k4,4 "./Face_ipv4_cache.tmp" "./Face_ipv4.tmp" >"./output/Face_ipv4.txt"
sort -b -u -k1,1 "./Face_ipv6_cache.tmp" "./Face_ipv6.tmp" >"./output/Face_ipv6.txt"
sort -b -u "./Face_domains.tmp" >"./output/Face_domains.txt"
sort -b -u "./Face_domains_abandoned.tmp" >"./output/Face_domains_abandoned.txt"
cnt_cache_tmpv4="$("${awk_tool}" 'END{printf "%d",NR}' "./Face_ipv4_cache.tmp" 2>/dev/null)"
cnt_cache_tmpv6="$("${awk_tool}" 'END{printf "%d",NR}' "./Face_ipv6_cache.tmp" 2>/dev/null)"
cnt_tmpv4="$("${awk_tool}" 'END{printf "%d",NR}' "./Face_ipv4.tmp" 2>/dev/null)"
cnt_tmpv6="$("${awk_tool}" 'END{printf "%d",NR}' "./Face_ipv6.tmp" 2>/dev/null)"
cnt_ipv4="$("${awk_tool}" 'END{printf "%d",NR}' "./output/Face_ipv4.txt" 2>/dev/null)"
cnt_ipv6="$("${awk_tool}" 'END{printf "%d",NR}' "./output/Face_ipv6.txt" 2>/dev/null)"
doh_end="$(date "+%s")"
doh_duration="$(((doh_end - doh_start1) / 60))m $(((doh_end - doh_start1) % 60))s"
